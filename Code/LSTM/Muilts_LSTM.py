...
import os
import sys
from Mmodels_LSTM import ELM
import pandas as pd
from sklearn.metrics import r2_score, mean_absolute_error, mean_squared_error


from Mget_data__LSTM import setup_seed
...
def practice(args, Pract):
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for (seq, labels) in Pract:
            ...
            optimizer.zero_grad()
            preds = model(seq,args)  # (n_outputs, batch_size, pred_output_size)将结果堆叠为张量
        ...
        scheduler.step()
        # validation
        val_loss = get_val_loss(args, model, Pract)
...
def practice_test(args, Pract, scaler):
