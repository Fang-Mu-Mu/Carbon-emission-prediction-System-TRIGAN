...
def nn_seq(args):
    ...
    def process(dataset, step_size, shuffle):
        ...
        seq = []
        for i in tqdm(range(0, len(dataset) - seq_len - output_size + 1, step_size)):  # 遍历数据
            train_seq = []
            for j in range(i, i + seq_len):  # 收集每个时间步的特征
                x = []
                for c in range(len(dataset[0])):  # 遍历所有特征（节点）
                    x.append(dataset[j][c])
                train_seq.append(x)

            # 收集标签
            train_labels = []
            for j in range(len(dataset[0])):  # 遍历所有特征（节点）
                train_label = []
            ...
            # 转换为张量
            train_seq = torch.FloatTensor(train_seq)  # 形状：(seq_len, num_nodes, in_channels)
            train_labels = torch.FloatTensor(train_labels)  # 形状：(pre_len, num_nodes)

            # 创建图的边关系
            edge_index = create_graph(num_nodes, train_seq.numpy())

            # 假设 MyDataset 和 DataLoader 正确处理图数据
            graph = Data(x=train_seq.permute(1, 0, 2), edge_index=edge_index,
                         y=train_labels)  # (num_nodes, seq_len, in_channels)
